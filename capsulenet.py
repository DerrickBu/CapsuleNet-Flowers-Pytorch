...
class Plotter:
    def plot_curve(self, sub_plot, params, train_column, valid_column, linewidth = 2, train_linestyle = "b-", valid_linestyle = "g-"):
        ...
        x_axis = np.arrange(epochs)
        sub_polt.plot(x_axis[valid_values > 0],  valid_values[valid_values > 0], valid_linestyle, linewidth = linewidth, label="train")
        return epochs

    def plot_learning_curves(self, params):
        curves_figure = plt.figure(figsize = (10, 4))
        sub_plot = curve_figure.add_subplot(1, 2, 1)
...
class CapsuleNet(nn.Module):
    ...
    def __init__(self, input_size, classes, routings):
        ...
        self.digitcaps = DenseCapsule(in_num_caps=32*104*104, in_dim_caps=8,
...
def test(model, test_loader, args):
    ...
    for x, y in test_loader:
        y = torch.zeros(y.size(0), 102).scatter_(1, y.view(-1, 1), 1.)
...
def train(model, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y) in enumerate(train_loader):  # batch training
            y = torch.zeros(y.size(0), 102).scatter_(1, y.view(-1, 1), 1.)  # change to one-hot coding
        ...
        histories['val_acc'] = np.append(histories['val_acc'], [val_loss])
...
def load_mnist(path='./flowers', download=False, batch_size=100, shift_pixels=2):
    """
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(path + '/train', transform=transforms.Compose([transforms.RandomResizedCrop(224), 
                                                                            transforms.ToTensor(),
                                                                            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])),
        batch_size=batch_size, shuffle=True, **kwargs)
    test_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(path + '/valid',transform=transforms.Compose([transforms.CenterCrop(224), 
                                                                           transforms.ToTensor(),
                                                                           transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])),
        batch_size=batch_size, shuffle=True, **kwargs)
...
if __name__ == "__main__":
    ...
    # define model
    model = CapsuleNet(input_size=[3, 224, 224], classes=102, routings=3)
    model.cuda()
    print(model)
